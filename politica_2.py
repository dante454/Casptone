#Aca se le agrega un radio para tomar desiciones si aceptar o no un pedido
#se define un radio desde el depot, si el pedido esta fuera de este no se acepta
...
def filtrar_pedidos_vencidos(pedidos_disponibles, minuto_actual, tiempo_limite=140):
    ...
    for pedido in pedidos_disponibles:
        if minuto_actual - pedido.minuto_llegada <= tiempo_limite:
            if minuto_actual < 720 and pedido.indicador != 0:
                continue
...
def flujo_ruteo(camion, simulacion):
    ...
    #2.5 Se descartan los pedidos fuera del buffer definido
...
# Definir un radio máximo (e.g., 5000 metros)
    radio_maximo = 10000
    pedidos_disponibles = filtrar_por_radio_depot(pedidos_disponibles, deposito, radio_maximo)
