#En esta politica se considera tener los tres camiones esten disponibles en las horas pick (a mitad del dia). 
#mitad del peak min 720
#se agrego que solo tome deliverys hasta ciertos min (modificable en filtrar pedidos vencidos)

#es casi el mismo % que para el caso normal, 
# al forzar que tomen pedidos de delivery nomas hasta mas tarde el timepo de respuesta baja 10min aprox 
...
def filtrar_pedidos_vencidos(pedidos_disponibles, minuto_actual, tiempo_limite=140):
    ...
    for pedido in pedidos_disponibles:
        if minuto_actual - pedido.minuto_llegada <= tiempo_limite:
            if minuto_actual < 900 and pedido.indicador != 0:
                continue
...
#nueva politica
def eliminar_puntos_si_reducen_distancia(ruta, simulacion, x_porcentaje=30, y_max_puntos=15):
    puntos_eliminados = 0
